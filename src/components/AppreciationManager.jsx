...
// ── Carte élève avec appréciation ─────────────────────────────────────────────
function StudentAppreciationCard({ student, grades, subjects, trimester, classAverage, onSave }) {
  ...
  const totalCoef = validGrades.reduce((sum, g) => {
    const subj = subjects.find(s => s.id === (g.subjectId || g.subject_id));
  ...
  const handleGenerate = async () => {
    if (validGrades.length === 0) { setError('Aucune note disponible'); return; }
    setLoading(true); setError(''); setSaved(false);
    try {
      const result = await generateAppreciation(student, validGrades, subjects, trimester, classAverage);
            ...
            {appreciation && (
              <>
                ...
                <span className="text-xs text-green-600 font-medium flex items-center gap-0.5">
                  <Check className="w-3 h-3" /> Générée
                </span>
              </>
            )}
          </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {!expanded && (
            <button onClick={(e) => { e.stopPropagation(); handleGenerate(); }}
              disabled={loading || validGrades.length === 0}
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-gray-50 pt-4 space-y-4">
              ...
              {validGrades.slice(0, 6).map((g, i) => {
                ...
                return (
                  <div key={i} className="bg-gray-50 rounded-lg px-2 py-1.5 flex items-center justify-between">
                    ...
                    <span className="text-xs font-black ml-1 flex-shrink-0" style={{ color: c }}>{g.value.toFixed(1)}</span>
                  </div>
                );
              })}
            </div>
          ...
          <div>
            ...
            <textarea value={appreciation} onChange={e => setAppreciation(e.target.value)} rows={3}
              placeholder="Cliquez sur 'Générer avec IA' pour créer une appréciation automatique..."
          ...
          <div className="flex items-center gap-2">
            <button onClick={handleGenerate} disabled={loading || validGrades.length === 0}
...
export default function AIAppreciations({
  ...
  calculateAverage, showNotification, updateGrade
}) {
  ...
  const classAverages = classStudents
    .map(s => parseFloat(calculateAverage(s.id, selectedTrimester)) || 0)
    .filter(v => v > 0);
  ...
  const handleGenerateAll = async () => {
    ...
    const eligible = classStudents.filter(s => getStudentGrades(s.id).some(g => g.value != null));
    if (eligible.length === 0) { showNotification('Aucun élève avec des notes'); return; }
    setGenerating(true); setGenProgress(0);
    let done = 0;
    for (const student of eligible) {
      try {
        const sg = getStudentGrades(student.id).filter(g => g.value != null);
        const enriched = { ...student, className: classes.find(c => c.id === (student.classId || student.class_id))?.name || '' };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <Sparkles className="w-6 h-6 text-purple-600" /> IA Appréciations
          </h2>
          <p className="text-sm text-gray-500 mt-0.5">Génération automatique avec Claude AI — Trimestre {selectedTrimester}</p>
        </div>
        <button onClick={handleGenerateAll} disabled={generating || !filterClass}
      ...
      {generating && (
        <div className="bg-purple-50 border border-purple-200 rounded-xl p-4">
          ...
          <div className="h-2 bg-purple-100 rounded-full overflow-hidden">
            <div className="h-full rounded-full bg-gradient-to-r from-purple-500 to-blue-500 transition-all duration-500" style={{ width: `${genProgress}%` }} />
          </div>
        </div>
      )}
      ...
      <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-xl p-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-purple-600 mt-0.5 leading-relaxed">
            Claude analyse les notes, points forts et faibles de chaque élève pour générer une appréciation personnalisée et professionnelle. Modifiez et sauvegardez chaque appréciation.
          </p>
        </div>
      </div>
      ...
      {classStudents.length === 0
        ? <div className="text-center py-16">
          ...
          <p className="text-gray-400">{filterClass ? 'Aucun élève dans cette classe' : 'Sélectionnez une classe pour commencer'}</p>
        </div>
        : <div className="space-y-3">
          {classStudents.map(student => {
            const enriched = { ...student, className: classes.find(c => c.id === (student.classId || student.class_id))?.name || '' };
            return (
              <StudentAppreciationCard key={student.id} student={enriched}
                grades={getStudentGrades(student.id)} subjects={subjects}
                trimester={selectedTrimester} classAverage={classAverage}
