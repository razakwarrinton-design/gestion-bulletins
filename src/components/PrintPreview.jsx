...
export default function PrintPreview({
  ...
  const studentStatus = average >= 12 ? { text: 'ADMIS(E)', color: '#059669' } : average >= 8 ? { text: 'À SUIVRE', color: '#d97706' } : { text: 'EN DIFFICULTÉ', color: '#dc2626' };
  ...
  // Tri des matières par note (pour points forts/faibles)
  const sortedGrades = [...studentGrades].filter(g => g.value != null).sort((a, b) => b.value - a.value);
  const strengths = sortedGrades.slice(0, 3).map(g => ({ name: subjects.find(s => s.id === (g.subjectId || g.subject_id))?.name || '?', value: g.value }));
  const weaknesses = sortedGrades.slice(-3).reverse().map(g => ({ name: subjects.find(s => s.id === (g.subjectId || g.subject_id))?.name || '?', value: g.value }));
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .result-value.status { font-size: 10pt; color: ${studentStatus.color}; }

  /* Section signatures */
  .signatures { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-top: 12px; }
  .sig-box { border: 1px solid #9ca3af; padding: 8px; height: 70px; text-align: center; }
  .sig-label { font-size: 8pt; font-weight: bold; text-transform: uppercase; color: #374151; margin-bottom: 4px; }
  ...
  <div class="results-band">
    ...
    <div class="result-cell">
      ...
      <div class="result-value status">${studentStatus.text}</div>
    </div>
  </div>
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .grade-bar { height: 100%; border-radius: 5px; transition: width 0.3s; }
  ...
  .sig-box { border: 1.5px dashed #cbd5e1; border-radius: 8px; padding: 10px; height: 68px; }
  .sig-label { font-size: 7.5pt; font-weight: 700; color: #64748b; text-transform: uppercase; }
  ...
  const generateModel3Html = () => {
    ...
    const evolSvg = evol.length > 1 ? `
      <svg width="280" height="80" viewBox="0 0 280 80">
        ${[0, 5, 10, 15, 20].map(v => `<line x1="30" y1="${65 - (v / 20) * 55}" x2="270" y2="${65 - (v / 20) * 55}" stroke="#f1f5f9" stroke-width="1"/>
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Corps */
  .body-grid { display: grid; grid-template-columns: 3fr 1.6fr; gap: 0; padding: 14px 18px; gap: 16px; }
  ...
  .sig-box { border: 1.5px dashed #cbd5e1; border-radius: 8px; padding: 10px; height: 65px; }
  .sig-label { font-size: 7.5pt; font-weight: 700; color: #64748b; text-transform: uppercase; }
  ...
  // ── Écouteur événement externe ────────────────────────────────────────────
  React.useEffect(() => {
    const handler = (e) => {
      const tmpl = e?.detail?.template || bulletinTemplate;
      const html = tmpl === 'model1' ? generateModel1Html() : tmpl === 'model2' ? generateModel2Html() : generateModel3Html();
          ...
          <div className="border-2 border-gray-200 rounded-xl p-4 hover:border-blue-400 hover:bg-blue-50 transition-all group">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-500 mb-3">Style ministère, entête institution, tableau des notes avec colonnes coefficient/total, zone signatures officielles.</p>
                <button
          ...
          <div className="border-2 border-gray-200 rounded-xl p-4 hover:border-emerald-400 hover:bg-emerald-50 transition-all group">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-500 mb-3">Design épuré, barres de progression colorées par note, graphique SVG des résultats par matière, carte élève avec initiales.</p>
                <button
          ...
          <div className="border-2 border-gray-200 rounded-xl p-4 hover:border-purple-400 hover:bg-purple-50 transition-all group">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-500 mb-3">Couverture premium dégradée, KPIs détaillés, tableau avec comparaison moy. classe, graphique d'évolution sur 3 trimestres, points forts/faibles auto.</p>
                <button
